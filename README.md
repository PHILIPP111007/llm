...
| `route_refresh_interval` | 4 токена | Как часто пересчитывается semantic route |
...
Здесь «блоки, используемые для внимания» — это `route_blocks=16`: attention работает максимум с 16 блоками, а не со всеми блоками префикса. Semantic route пересчитывается раз в 4 токена и принудительно обновляется при переходе к новому блоку. Значение 5 было бы другим, более агрессивным режимом маршрутизации; в текущем экспериментальном конфиге используется 16.
