...
### 6.7. Chunked speed/PPL после оптимизации cache

После добавления dequantized route-cache и кольцевого буфера локального окна
выполнен быстрый `chunked` benchmark для hierarchical cosine routing.
Использовались `block_size=256`, `route_blocks=16`, `beam_width=16`,
`summary_parts=4`, `local_window=256`, `route_refresh_interval=64` и
`chunk_size=1024`. Dense baseline не запускался.

| Контекст | Время | Throughput | Mean NLL | PPL |
|---:|---:|---:|---:|---:|
| 32,000 | 4.05 s | 7,899 tok/s | 6.210 | 497.63 |
| 100,000 | 13.25 s | 7,547 tok/s | 6.495 | 661.62 |

Статистика индекса:

| Контекст | Route calls по всем слоям | Среднее узлов на route |
|---:|---:|---:|
| 32,000 | 496 | 1,072 |
| 100,000 | 1,552 | 1,617 |

Относительно предыдущего hierarchical prefill benchmark после оптимизации
получено примерно `2.6x` ускорение на 32K и `2.9x` на 100K. Время растёт почти
линейно: увеличение контекста в 3.125 раза увеличило время примерно в 3.27
раза.

PPL `497.63` на 32K и `661.62` на 100K нельзя сравнивать с native-context PPL
`21.5`. Эти значения получены вне обученного контекста Pythia, с chunked
representative routing и RoPE за пределами native range. Поэтому данный тест
фиксирует speed path и масштабирование runtime, но не доказывает качество
модели на сверхдлинных текстах.

### 6.8. Качество на длинных контекстах
